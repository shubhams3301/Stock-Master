...
class productClass:
    def __init__(self,root):
        ...
        cmb_satus=ttk.Combobox(product_Frame,textvariable=self.var_status,values=("Active","Inactive"),state='readonly',justify=CENTER,font=("goudy old style",15))
        ...
        self.product_table.heading("status",text="Satus")
    ...
    def add(self):
        ...
        try:
            if self.var_cat.get()=="Select"or self.var_cat.get()=="Empty" or self.var_sup.get()=="Select" or self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Product already, try different",parent=self.root)
                else:
                  ...
                  con.commit()
                  messagebox.showinfo("Success","Product Added Sucessfully",parent=self.root)
                  self.show()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to : {str(ex)}",parent=self.root)
